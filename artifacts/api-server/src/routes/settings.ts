...
const settingsInputSchema = z.object({
  ...
  profileImage: z.string().nullable().optional(), // รองรับการบันทึกรูป
});
...
router.get("/", async (req: AuthRequest, res: Response) => {
  ...
  const rows = await db.select().from(salarySettingsTable).where(eq(salarySettingsTable.userId, userId)).limit(1);
...
router.put("/", async (req: AuthRequest, res: Response) => {
  ...
  const data = { ...parsed.data, userId, updatedAt: new Date() };

  const existing = await db.select().from(salarySettingsTable).where(eq(salarySettingsTable.userId, userId)).limit(1);

  let row;
  if (existing.length > 0) {
    const updated = await db.update(salarySettingsTable).set(data).where(eq(salarySettingsTable.userId, userId)).returning();
    row = updated[0];
  } else {
    const inserted = await db.insert(salarySettingsTable).values(data).returning();
    row = inserted[0];
  }
