...
router.post("/", async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (isNaN(salaryNum)) {
      res.status(400).json({ message: "เงินเดือนต้องเป็นตัวเลข" });
      return;
    }

    if (pdfUrl) {
      try { new URL(pdfUrl); } catch {
        res.status(400).json({ message: "URL PDF ไม่ถูกต้อง" });
      return;
      }
    }
...
router.put("/:id", async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (result.rows.length === 0) {
      res.status(404).json({ message: "ไม่พบสลิปที่ต้องการแก้ไข" });
      return;
    }
...
router.delete("/:id", async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (result.rows.length === 0) {
      res.status(404).json({ message: "ไม่พบสลิปที่ต้องการลบ" });
      return;
    }
